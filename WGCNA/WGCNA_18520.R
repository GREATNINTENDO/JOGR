library("WGCNA")
library(tidyverse)
AssayData=data.table::fread("exprSet_GSE18520.txt")
AssayData=column_to_rownames(AssayData,var = "V1")
WGCNA_matrix = t(AssayData[order(apply(AssayData,1,mad), decreasing = T),])#mad代表绝对中位差
...
#traitData=read.table(file="traitdata_GSE37837.txt",header = T,check.names = F)

traitData=data.frame(Cancer=c(rep(1,53),rep(0,10)),
                     Control=c(rep(0,53),rep(1,10)))
...
MEDissThres = 0.2 #剪切高度可修改  MEDissThres =0.15#0.15剪切高度可修改 ###可以完成相似模块的合并,剪切高度是0.15,
